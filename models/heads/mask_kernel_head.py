...
LastEditTime: 2022-02-16 21:29:54
Description: file content
FilePath: /iProject/models/heads/mask_kernel_head.py
'''
...
from models.misc import multi_apply, points_nms, dice_loss, matrix_nms
from models.nninit import normal_init, bias_init_with_prob
from models.losses.focal_loss import FocalLoss
from datasets.piplines import imrescale
...
class MaskKernelHead(nn.Module):
    ...
    def __init__(
            ...
            num_grids=None,
            ins_out_channels=64):
        super(MaskKernelHead, self).__init__()
        ...
        self.loss_cate = FocalLoss(use_sigmoid=True, gamma=2.0, alpha=0.5, loss_weight=1.0)
    ...
    def loss(self, cate_preds, kernel_preds, ins_pred, gt_bbox_list, gt_label_list, gt_mask_list, img_metas, cfg=None, gt_bboxes_ignore=None):
        
        mask_feat_size = ins_pred.size()[-2:]
        ins_label_list, cate_label_list, ins_ind_label_list, grid_order_list = multi_apply(self.get_gt_label, gt_bbox_list, gt_label_list, gt_mask_list, mask_feat_size=mask_feat_size)

        # ins_labels = []
        # for ins_labels_level in zip(*ins_label_list):
        #     tmp = []
        #     for ins_labels_level_img in ins_labels_level:
        #         tmp.append(ins_labels_level_img)
        #     tmp = torch.cat(tmp, 0)
        #     ins_labels.append(tmp)
        ...
        #         tmp.append(kernel_preds_level_img.view(kernel_preds_level_img.shape[0], -1)[:, grid_orders_level_img])
        #     kernel_preds_copy.append(tmp)
        # kernel_preds = kernel_preds_copy
        ...
        #         tmp.append(ins_ind_labels_level_img.flatten())
        #     tmp = torch.cat(tmp)
        #     ins_ind_labels.append(tmp)
    ...
    def get_gt_label(self, gt_bboxes_raw, gt_labels_raw, gt_masks_raw, mask_feat_size):
